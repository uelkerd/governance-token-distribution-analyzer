...
class APIValidator:
    ...
    def validate_infura_api(self) -> Tuple[bool, str]:
        ...
        if (
            not self.config.infura_project_id
            or self.config.infura_project_id == "your_infura_project_id_here"
        ):
    ...
    def validate_graph_api(self) -> Tuple[bool, str]:
        ...
        if (
            not self.config.graph_api_key
            or self.config.graph_api_key == "YOUR_GRAPH_API_KEY_HERE"
        ):
        ...
        try:
            ...
            query = {"query": "{ proposals(first: 1) { id } }"}
            ...
            if response.status_code == 200:
                ...
                if "data" in data and not data.get("errors"):
                    ...
                    return (
                        True,
                        f"API key valid but query limited: {data['errors'][0].get('message', 'Unknown')}",
                    )
                else:
    ...
    def run_all_validations(self) -> Dict[str, Tuple[bool, str]]:
        ...
        print(
            "🔍 Validating API Connections for Governance Token Distribution Analyzer"
        )
        print("=" * 80)
    ...
    def print_summary(self, results: Dict[str, Tuple[bool, str]]):
        ...
        if successful == total:
            print(
                "🚀 All API connections are ready! You can proceed with live data implementation."
            )
        else:
                ...
                if not success:
                    if "not configured" in message:
                        if "Graph" in test_name:
                            print(
                                f"   • Sign up for The Graph API at https://thegraph.com/studio/"
                            )
                        else:

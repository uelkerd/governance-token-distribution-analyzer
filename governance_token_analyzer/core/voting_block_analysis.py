...
class VotingBlockAnalyzer:
    ...
    def load_voting_data(self, proposals: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            for proposal in proposals:
                ...
                for vote in votes:
                    if 'voter' not in vote or 'support' not in vote:
                        logger.warning(f"Skipping vote in proposal {proposal_id}: missing required fields")
                        continue
            ...
            logger.info(f"Loaded voting data for {len(voting_data)} addresses across {len(proposals)} proposals")
        ...
        try:
            ...
            for i in range(n_addresses):
                ...
                for j in range(i+1, n_addresses):
                    ...
                    agreements = sum(1 for p in common_proposals if votes1[p] == votes2[p])
            ...
            logger.info(f"Calculated voting similarity for {n_addresses} addresses with minimum overlap of {min_overlap} proposals")
            return sim_df
            
        except Exception as e:
            logger.error(f"Failed to calculate voting similarity: {e}")
            raise HistoricalDataError(f"Failed to calculate voting similarity: {e}") from e
    
    def identify_voting_blocks(self, similarity_threshold: float = 0.8) -> List[List[str]]:
        """
        ...
        try:
            ...
            for i, addr1 in enumerate(addresses):
                for j, addr2 in enumerate(addresses):
                    if i < j and self.address_similarity.loc[addr1, addr2] >= similarity_threshold:
                        G.add_edge(addr1, addr2, weight=self.address_similarity.loc[addr1, addr2])
            ...
            logger.info(f"Identified {len(voting_blocks)} voting blocks with similarity threshold of {similarity_threshold}")
    ...
    def calculate_voting_power(self, voting_blocks: List[List[str]], token_balances: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            for i, block in enumerate(voting_blocks):
                ...
                block_percentage = (block_tokens / total_tokens * 100) if total_tokens > 0 else 0
            ...
            raise HistoricalDataError(f"Failed to calculate voting block power: {e}") from e
    ...
    def get_block_voting_patterns(self, block: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for proposal_id in all_proposals:
                ...
                for addr in block:
                    if addr in self.voting_history and proposal_id in self.voting_history[addr]:
            ...
            avg_participation = sum(p["participation_pct"] for p in proposal_stats.values()) / len(proposal_stats)
            avg_consensus = sum(p["consensus_pct"] for p in proposal_stats.values()) / len(proposal_stats)
            ...
            logger.info(f"Analyzed voting patterns for block with {len(block)} addresses across {len(all_proposals)} proposals")
            return result
            
        except Exception as e:
            logger.error(f"Failed to analyze block voting patterns: {e}")
            raise HistoricalDataError(f"Failed to analyze block voting patterns: {e}") from e
    ...
    def visualize_voting_blocks(self, token_balances: Optional[Dict[str, float]] = None) -> plt.Figure:
        ...
        try:
            if self.address_similarity is None or self.address_similarity.empty:
                ...
                ax.text(0.5, 0.5, "No voting block data available", ha='center', va='center', transform=ax.transAxes)
                return fig
            
            if not self.voting_blocks:
                logger.warning("No voting blocks identified for visualization")
                fig, ax = plt.subplots(figsize=(10, 8))
                ax.text(0.5, 0.5, "No voting blocks identified", ha='center', va='center', transform=ax.transAxes)
                return fig
            ...
            for node, (x, y) in pos.items():
                ax.scatter(x, y, s=node_sizes[list(G.nodes()).index(node)], color=color_map[node], alpha=0.7)
            
            # Draw edges
            for u, v, data in G.edges(data=True):
                ax.plot([pos[u][0], pos[v][0]], [pos[u][1], pos[v][1]], 'k-', alpha=0.2 + data['weight'] * 0.6)
            ...
            for i, block in enumerate(self.voting_blocks):
                ...
                if xs and ys:
                    ...
                    ax.text(block_centers[i][0], block_centers[i][1], f"Block {i+1}", 
            ...
            logger.info(f"Created visualization for {len(self.voting_blocks)} voting blocks with {len(G.nodes())} addresses")
            return fig
...
def detect_voting_anomalies(
    ...
    try:
        ...
        avg_participation = sum(proposal_participation.values()) / len(proposal_participation) if proposal_participation else 0
                    ...
                    if len(common_props) >= 3:
                        ...
                        if always_same:
                            anomalies['coordinated_voting'].append({
                                ...
                                'balances': [balance_map.get(addr1, 0), balance_map.get(addr2, 0)]
                            })
            ...
            if len(sorted_voters) >= 10:
                ...
                if top_sentiment != bottom_sentiment:
                    anomalies['whale_against_community'].append({
                        ...
                        'bottom_against_percentage': (bottom_against / len(bottom_votes)) * 100 if bottom_votes else 0
                    })

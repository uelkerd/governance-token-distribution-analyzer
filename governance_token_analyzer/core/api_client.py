...
class APIClient:
    ...
    def __init__(self, etherscan_api_key: str = DEFAULT_ETHERSCAN_API_KEY, 
        ...
        if not self.etherscan_api_key:
            logger.warning("No Etherscan API key provided. Set ETHERSCAN_API_KEY environment variable.")
        if not self.infura_api_key:
            logger.warning("No Infura API key provided. Set INFURA_API_KEY environment variable.")
    
    def get_token_holders(self, protocol: str, limit: int = 100, use_real_data: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def get_governance_proposals(self, protocol: str, limit: int = 10, use_real_data: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def get_protocol_data(self, protocol: str, use_real_data: bool = False) -> Dict[str, Any]:
        ...
        for proposal in proposals:
            ...
            votes = self.get_governance_votes(protocol, proposal_id, use_real_data=use_real_data)
    ...
    def _fetch_token_holders(self, protocol: str, token_address: str, limit: int) -> List[Dict[str, Any]]:
        """
    ...
    def _fetch_compound_token_holders(self, token_address: str, limit: int) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            params = {
                "apiKey": DEFAULT_ETHPLORER_API_KEY,
                "limit": limit
            }
    ...
    def _fetch_uniswap_token_holders(self, token_address: str, limit: int) -> List[Dict[str, Any]]:
        """
    ...
    def _fetch_aave_token_holders(self, token_address: str, limit: int) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if "data" in data and "proposals" in data["data"]:
                for proposal_data in data["data"]["proposals"]:
                    ...
                    title = description.split("\n")[0] if description else f"Proposal {proposal_data.get('id', 'Unknown')}"
                    
                    # Parse votes
                    for_votes = float(proposal_data.get("forVotes", "0")) / (10 ** 18)
                    against_votes = float(proposal_data.get("againstVotes", "0")) / (10 ** 18)
                    abstain_votes = float(proposal_data.get("abstainVotes", "0")) / (10 ** 18)
                    total_votes = for_votes + against_votes + abstain_votes
                    
                    # Calculate vote percentages
                    for_percentage = (for_votes / total_votes * 100) if total_votes > 0 else 0
                    against_percentage = (against_votes / total_votes * 100) if total_votes > 0 else 0
            ...
            # Fallback to sample data if API call fails
            logger.warning("Falling back to sample data for Compound governance proposals")
        ...
        try:
            ...
            if "data" in data and "proposals" in data["data"]:
                for proposal_data in data["data"]["proposals"]:
                    ...
                    created_time = datetime.fromtimestamp(int(proposal_data.get("createdTimestamp", 0)))
            ...
            # Fallback to sample data if API call fails
            logger.warning("Falling back to sample data for Uniswap governance proposals")
            ...
            if "data" in data and "proposals" in data["data"]:
                for proposal_data in data["data"]["proposals"]:
                    ...
                    if proposal_data.get("executionTime"):
                        try:
                            created_time = datetime.fromtimestamp(int(proposal_data.get("executionTime", 0)))
                            created_at = created_time.isoformat()
    ...
    def _fetch_compound_votes(self, proposal_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "data" in data and "votes" in data["data"]:
                for vote_data in data["data"]["votes"]:
                    ...
                    if vote_data.get("blockTimestamp"):
                        try:
                            voted_time = datetime.fromtimestamp(int(vote_data.get("blockTimestamp", 0)))
            ...
            logger.error(f"Error fetching votes for Compound proposal {proposal_id}: {str(e)}")
            # Fallback to sample data if API call fails
            logger.warning(f"Falling back to sample data for Compound proposal {proposal_id} votes")
    ...
    def _fetch_uniswap_votes(self, proposal_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "data" in data and "votes" in data["data"]:
                for vote_data in data["data"]["votes"]:
                    ...
                    if vote_data.get("timestamp"):
                        try:
                            voted_time = datetime.fromtimestamp(int(vote_data.get("timestamp", 0)))
                            voted_at = voted_time.isoformat()
                        except (ValueError, TypeError):
                            pass
            ...
            logger.error(f"Error fetching votes for Uniswap proposal {proposal_id}: {str(e)}")
            # Fallback to sample data if API call fails
            logger.warning(f"Falling back to sample data for Uniswap proposal {proposal_id} votes")
    ...
    def _fetch_aave_votes(self, proposal_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching votes for Aave proposal {proposal_id}: {str(e)}")
            # Fallback to sample data if API call fails
            logger.warning(f"Falling back to sample data for Aave proposal {proposal_id} votes")
    ...
    def _generate_sample_holder_data(self, protocol: str, count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(regular_count):
            ...
            holder = {
                ...
                'last_transaction': (datetime.now() - timedelta(days=random.randint(1, 60))).isoformat(),
    ...
    def _generate_sample_proposal_data(self, protocol: str, count: int) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            status_weights = [0.1, 0.5, 0.2, 0.1, 0.1]  # More passed proposals than failed
    ...
    def _generate_sample_vote_data(self, protocol: str, proposal_id: int) -> List[Dict[str, Any]]:
        ...
        voting_powers = self._generate_power_law_distribution(num_votes, total_supply * 0.4, 1.5)

...
def main():
    ...
    for protocol, holders_df in token_data.items():
        ...
        if concentration_metrics:
            logger.info(f"{protocol.capitalize()} - Gini Coefficient: {concentration_metrics.get('gini_coefficient', 'N/A')}")
            logger.info(f"{protocol.capitalize()} - Top 10 Concentration: {concentration_metrics.get('concentration_ratio_top10', 'N/A')}%")
    ...
    for protocol, holders_df in token_data.items():
        ...
        holder_size_result = participation_analysis.analyze_participation_by_holder_size(
            governance_data[protocol],
            holders_df,
            protocol_name=protocol
        )
        ...
        if participation_metrics:
            logger.info(f"{protocol.capitalize()} - Participation Rate: {participation_metrics.get('overall_participation_rate', 'N/A')}%")
            logger.info(f"{protocol.capitalize()} - Total Votes: {participation_metrics.get('total_votes', 'N/A')}")

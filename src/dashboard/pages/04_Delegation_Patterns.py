        ...
        if 'delegation_network' in delegation_data:
            ...
            if len(G.nodes()) > 0:
                ...
                delegator_trace = go.Scatter(
                    ...
                    text=[f"Delegator: {node[:6]}...{node[-4:]}<br>Voting Power: {G.nodes[node].get('voting_power', 0):,.2f}" for node in delegator_nodes],
                    hoverinfo='text',
                    name='Delegators'
                )
                ...
                delegatee_trace = go.Scatter(
                    ...
                    text=[f"Delegatee: {node[:6]}...{node[-4:]}<br>Delegated Power: {G.nodes[node].get('delegated_power', 0):,.2f}" for node in delegatee_nodes],
                    hoverinfo='text',
                    name='Delegatees'
                )

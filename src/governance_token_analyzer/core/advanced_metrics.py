...
def calculate_top_percentiles(balances: List[float], percentiles: Optional[List[int]] = None) -> Dict[str, float]:
    ...
    Args:
        balances: List of token balances (should be sorted in descending order)
        percentiles: List of percentiles to calculate (default: [1, 5, 10, 20, 50])

    Returns:
        Dictionary mapping percentile to percentage of tokens held

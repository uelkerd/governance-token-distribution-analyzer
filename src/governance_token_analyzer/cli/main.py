...
@click.option(
    "--historical", is_flag=True, help="Analyze historical delegation patterns"
)
@click.option(
    "--min-threshold",
    type=float,
    default=0.01,
    help="Minimum threshold for significant delegations",
)
@click.option(
    "--shift-threshold",
    type=float,
    default=0.1,
    help="Threshold for considering a change significant",
)
@click.option(
    "--data-dir",
    type=str,
...
@click.option(
    "--output-dir", type=str, default="reports", help="Directory to save output"
)
@click.option(
    "--output-format", type=str, default="json", help="Output format (json, csv)"
)
    ...
    try:
        ...
        if historical:
            ...
            results = dpa.analyze_historical_delegation_patterns(
                snapshots, min_threshold=min_threshold, shift_threshold=shift_threshold
            )

            # Output filename
            output_file = os.path.join(
                output_dir, f"{protocol}_historical_delegation_analysis.{output_format}"
            )
        else:
            # Get latest snapshot
            snapshots = data_manager.get_snapshots(protocol)
            ...
            results = dpa.analyze_delegation_patterns(
                latest_snapshot["data"], min_threshold=min_threshold
            )

            # Output filename
            output_file = os.path.join(
                output_dir, f"{protocol}_delegation_analysis.{output_format}"
            )
        ...
        if output_format == "json":
            ...
            if historical:
                ...
                metrics_file = os.path.join(
                    output_dir, f"{protocol}_delegation_metrics.csv"
                )
                pd.DataFrame([metrics]).to_csv(metrics_file, index=False)

                # Save delegatees if any
                if delegatees:
                    delegatees_file = os.path.join(
                        output_dir, f"{protocol}_key_delegatees.csv"
                    )

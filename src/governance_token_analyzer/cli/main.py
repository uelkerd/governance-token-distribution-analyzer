...
def historical_analysis(protocol, snapshots, interval, metric, output_dir, plot):
    ...
    try:
        ...
        generated_snapshots = historical_data.simulate_historical_data(
            protocol=protocol, num_snapshots=snapshots, interval_days=interval, data_manager=data_manager, seed=42
        )
...
@click.argument("protocol", type=ProtocolChoice())
@click.option(
    "--format", type=click.Choice(["json", "csv", "excel"]), default="json", help="Export format (default: json)"
)
@click.option(
...
@click.argument("protocol", type=ProtocolChoice())
@click.option(
    "--format", type=click.Choice(["html", "pdf", "markdown"]), default="html", help="Report format (default: html)"
)
@click.option(

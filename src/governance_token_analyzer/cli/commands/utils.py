...
def filter_numeric_values(
    protocols: List[str], data: Dict[str, Dict[str, Any]], metric: str
...
def create_bar_chart(protocols: List[str], values: List[float], metric: str, output_file: str) -> None:
    ...
    bars = plt.bar(
        display_names, values, color=["#3498db", "#2ecc71", "#e74c3c", "#f39c12", "#9b59b6"][: len(protocols)]
    )
    ...
    for bar in bars:
        ...
        plt.text(
            bar.get_x() + bar.get_width() / 2.0, height + 0.02, f"{height:.3f}", ha="center", va="bottom", fontsize=10
        )
...
def collect_protocol_data_safely(
    metrics_collector: MetricsCollector, protocol: str, limit: int = 1000
) -> Dict[str, Any]:
    """

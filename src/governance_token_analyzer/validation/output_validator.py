...
class OutputValidator:
    ...
    def _validate_output_structure(self, data: Dict[str, Any], result: Dict[str, Any]) -> None:
        ...
        required_fields = [
            "protocol", "timestamp", "data_source", "total_holders_analyzed", "metrics"
        ]
        
        required_metrics = [
            "gini_coefficient", "herfindahl_index", "nakamoto_coefficient",
            "top_percentile_concentration", "lorenz_curve"
        ]
        ...
        if gini is not None:
            ...
            if gini_range and gini_range[0] <= gini <= gini_range[1]:
                ...
                result["warnings"].append(
                    f"Gini coefficient outside expected range {gini_range}: {gini:.4f}"
                )
        ...
        try:
            ...
            for protocol in protocols_compared:
                if protocol in results:
                    protocol_validation = self.validate_analysis_output(
    ...
    def _validate_ranking_consistency(self, results: Dict[str, Any], ranking: List[str], 
                                    primary_metric: str, validation_result: Dict[str, Any]) -> None:
    ...
    def generate_validation_report(self, validation_results: List[Dict[str, Any]]) -> str:
        ...
        for i, result in enumerate(validation_results, 1):
            ...
            report_lines.extend([
                f"## Validation {i}: {protocol} - {status}",
                ""
            ])
